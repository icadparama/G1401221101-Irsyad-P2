...
---
TUGAS MPDW Kelompok 6

Kelompok 6 mendapatkan tema lingkungan, dimana kita mengambil data rata-rata suhu yang berada pada DKI jakarta dan rentang waktu yang diambil merupakan 1 Januari 2023 - 15 May 2024 yaitu berupa 500 periode, saya mendapat bagian dari tanggal 20 Juni 2023 - 26 Oktober.
```{r}
library(readxl)
data <- read_excel("C:/Users/Irsyad Parama/Downloads/kab dki jakarta 1 jan 2023 - 15 mei 2024.xlsx", sheet = "201-300")
head(data)
```
...
```{r}
ts.plot(data.ts, xlab="Waktu", ylab="Tavg", main="Time Series Plot")
points(data.ts)
```

```{r}
training_ma <- data[1:80,]
testing_ma <- data[81:99,]
View(testing_ma)
...
training_ma$Waktu <- as.Date(training_ma$Waktu, format = "%d-%m-%Y")
testing_ma$Waktu <- as.Date(testing_ma$Waktu, format = "%d-%m-%Y")
training_ma <- training_ma[order(training_ma$Waktu), ]
testing_ma <- testing_ma[order(testing_ma$Waktu), ]

# Plot data with ggplot2
ggplot() + 
  geom_line(data = training_ma, aes(x = Waktu, y = `Tavg`, col = "Data Latih", group = 1)) +
  geom_line(data = testing_ma, aes(x = Waktu, y = `Tavg`, col = "Data Uji", group = 1)) +
  labs(x = "Periode Waktu", y = "Tavg", color = "Legend") +
...
```{r}
training_exp <- data[1:80,]
testing_exp <- data[81:99,]

train_ma.exp <- ts(training_exp$'Tavg')
test_ma.exp <- ts(testing_exp$'Tavg')
```
...
```{r}
ggplot() + 
  geom_line(data = training_exp, aes(x = Waktu, y = `Tavg`, col = "Data Latih", group = 1)) +
  geom_line(data = testing_exp, aes(x = Waktu, y = `Tavg`, col = "Data Uji", group = 1)) +
  labs(x = "Periode Waktu", y = "Tavg", color = "Legend") +
  scale_colour_manual(name = "Legend", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + 
...
```{r}
time_series_data <- ts(training_exp$'Tavg', start = c(2012, 27), frequency = 52) 
...
```{r}
resid2<-training_exp$"Tavg"-ramalan2$fitted
head(resid2)
```
...
```{r}
residopt<-training_exp$"Tavg"-ramalanopt$fitted
head(residopt)
```
...
```{r}
accuracy(ramalanopt,testing_exp$'Tavg')
```
...
```{r}
selisihdes1 <- ramalandes1$mean - testing_exp$`Tavg`
selisihdes1
```
...
SSEtestingdes1
MSEtestingdes1 <- SSEtestingdes1 / length(testing_exp$`Tavg`)
MAPEtestingdes1 <- sum(abs(selisihdes1 / testing_exp$`Tavg`) * 100) / length(testing_exp$`Tavg`)

selisihdes2 <- ramalandes2$mean - testing_exp$`Tavg`
selisihdes2
```

```{r}
SSEtestingdes2 <- sum(selisihdes2^2)
MSEtestingdes2 <- SSEtestingdes2 / length(testing_exp$`Tavg`)
MAPEtestingdes2 <- sum(abs(selisihdes2 / testing_exp$`Tavg`) * 100) / length(testing_exp$`Tavg`)

selisihdesopt <- ramalandesopt$mean - testing_exp$`Tavg`
selisihdesopt
```

```{r}
SSEtestingdesopt <- sum(selisihdesopt^2)
MSEtestingdesopt <- SSEtestingdesopt / length(testing_exp$`Tavg`)
MAPEtestingdesopt <- sum(abs(selisihdesopt / testing_exp$`Tavg`) * 100) / length(testing_exp$`Tavg`)
...
trainter <- ts(trainwin$'Tavg', frequency = 10) #start = c(tahun mulai, minggu ke-berapa), frequency = 52 karena per minggufrequency=52)
testter <- ts(testwin$'Tavg', frequency = 10) #start = c(tahun mulai, minggu ke-berapa), frequency = 52 karena per minggu52)
```
